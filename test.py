...
def render_high_graph(data, threshold=1000, max_value=2000, width=60):
    blocks = "▁▂▃▄▅▆▇█"
    graph = []
    for val in data[-width:]:  # Limit to last `width` readings
        try:
            val = float(val)
        except:
            val = 0.0
        if val >= threshold:
            ...
            graph.append(" ")  # Blank for low readings
...
def build_environment_panel():
    try:
        ...
        graph = render_high_graph(co2_history, threshold=1500)  # ← This is the call
        ...
        lines = [
            ...
            "           └───────────────────────────────────────────────",
            "           Time →  [Last 24h]",
            f"           Max: {max_values['Environment']['CO₂']:.1f} ppm"
        ]
        body = Text("\n".join(lines), style="green")
    except Exception as e:
        body = Text(f"[red]Sensor Error: {e}[/red]")
    return Panel(body, title="🌍 CO₂ High Readings", border_style="grey37")
