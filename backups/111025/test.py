...
def build_combined_tent_environment_panel():
    try:
        ...
        body = Text("\n").join(lines)
        return Panel(body, title="🌡 Tent + Environment", border_style="grey37")
...
def build_grow_panel():
    ...
    for entry in plants_data:
        ...
        height = height_bar(height_val, prev_val)
        harvest_weight = entry.get("harvest_weight", 0.0)
        harvest = f"{harvest_weight:.1f}g" if harvest_weight else "[Pending]"

        table.add_row(name, type_, breeder,prop_date, repot, progress, last_watered, height, harvest)
...
def build_dashboard():
    ...
    second_row_panels = [
        ...
        build_i2c_panel(),
        build_combined_tent_environment_panel()
    ]
